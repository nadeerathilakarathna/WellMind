            ...
            if timestamps:
                ...
                if facial_valid_data:
                    ...
                    ax.plot(facial_times, facial_values, marker="o", color="#FF5722", 
                        linewidth=1.5, markersize=3, label="Facial Expression Stress", alpha=1, 
                ...
                if keystroke_valid_data:
                    ...
                    ax.plot(keystroke_times, keystroke_values, marker="o", color="#4CAF50", 
                        linewidth=1.5, markersize=3, label="Keystroke Stress", alpha=1, 
                        linestyle='-', drawstyle='default')
...
class DashboardScreen(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.stress_cards = {}
        self.feedback_like_label = None
        self.feedback_unlike_label = None
    ...
    def start_auto_refresh(self):
        ...
        if not self.is_destroyed:
            self.auto_refresh_job = self.after(10000, self.auto_refresh_data)  # 10 seconds
    ...
    def auto_refresh_data(self):
        ...
        try:
            if not self.is_destroyed and self.winfo_exists():
                print(f"Auto-refreshing dashboard data at {datetime.now().strftime('%H:%M:%S')}")
                
                # Update stress metrics cards
                self.update_stress_metrics()
                
                # Update feedback counts
                self.update_feedback_counts()
                
                # Update stress graph with current selected period
                current_period = self.time_period_dropdown.get()
                self.update_stress_graph(current_period)
            ...
            # Try to continue with next refresh
            self.start_auto_refresh()

    def update_stress_metrics(self):
        """Update the stress metrics cards"""
        try:
            metrics = get_stress_metrics()
            if metrics and self.stress_cards:
                for title, value in metrics.items():
                    if title in self.stress_cards:
                        # Update the value label in the card
                        card, value_label = self.stress_cards[title]
                        value_label.configure(text=str(value))
        except Exception as e:
            print(f"Error updating stress metrics: {e}")
    ...
    def update_feedback_counts(self):
        ...
        try:
            ...
            if self.feedback_like_label:
                self.feedback_like_label.configure(text=str(likes))
            if self.feedback_unlike_label:
                self.feedback_unlike_label.configure(text=str(unlikes))
        except Exception as e:
            print(f"Error updating feedback counts: {e}")
        ...
        try:
            ...
            if self.recommendations_frame:
                for widget in self.recommendations_frame.winfo_children():
                    if isinstance(widget, ctk.CTkFrame) and widget != self.rec_title_frame:
                        widget.destroy()
    ...
    def display_recommendations(self):
        """Display recommendations in the recommendations frame"""
        try:
            recommendations = fetch_recent_recommendations()
            now = datetime.now()
            ...
            if not recommendations:
                ...
                for rec_data in recommendations:
                    ...
                    card = ctk.CTkFrame(self.recommendations_frame, fg_color="#FFFFFF", corner_radius=10)
                    card.pack(padx=10, pady=5, fill="x")
                    card.grid_columnconfigure(0, weight=1)
                    card.grid_columnconfigure(1, weight=0)
    ...
    def destroy(self):
        """Override destroy to clean up auto-refresh"""
    ...
    def update_stress_graph(self, selected_period):
        ...
        try:
            ...
            print(f"Error updating stress graph: {e}")
    ...
    def build_ui(self):
        ...
        if metrics:
            for title, value in metrics.items():
                ...
                title_label = ctk.CTkLabel(card, text=title, font=ctk.CTkFont("Poppins", 12, "bold"), text_color="#000")
                title_label.pack(pady=(10, 0))
                
                value_label = ctk.CTkLabel(card, text=str(value), font=ctk.CTkFont("Poppins", 14), text_color="#254D70")
                value_label.pack(pady=(0, 10))
                
                # Store reference to the card and value label for updates
                self.stress_cards[title] = (card, value_label)
        else:
            # Optional fallback if metrics not found
            ctk.CTkLabel(card_frame, text="No stress metrics available", font=ctk.CTkFont("Poppins", 14)).pack(pady=20)
        ...
        like_frame.pack(side="left", padx=(0, 15))
        ctk.CTkLabel(like_frame, image=thumb_up_ctk, text="", width=16, height=16).pack(side="left")
        self.feedback_like_label = ctk.CTkLabel(like_frame, text=str(likes), font=ctk.CTkFont("Poppins", 12), text_color="#254D70", padx=6)
        self.feedback_like_label.pack(side="left")

        # Unlike count
        unlike_frame = ctk.CTkFrame(feedback_icon_frame, fg_color="transparent")
        unlike_frame.pack(side="left")
        ctk.CTkLabel(unlike_frame, image=thumb_down_ctk, text="", width=16, height=16).pack(side="left")
        self.feedback_unlike_label = ctk.CTkLabel(unlike_frame, text=str(unlikes), font=ctk.CTkFont("Poppins", 12), text_color="#254D70", padx=6)
        self.feedback_unlike_label.pack(side="left")
        ...
        # Recommendations section
        self.recommendations_frame = ctk.CTkFrame(self.scrollable_frame, fg_color="#333333", corner_radius=12)
        self.recommendations_frame.pack(fill=ctk.X, padx=25, pady=(0, 20))
        ...
        self.rec_title_frame = ctk.CTkFrame(self.recommendations_frame, fg_color="transparent")
        self.rec_title_frame.pack(anchor="w", padx=10, pady=(10, 5))

        ctk.CTkLabel(self.rec_title_frame, image=rec_icon_ctk, text="", width=20, height=20).pack(side="left")
        ctk.CTkLabel(self.rec_title_frame, text="Recommendations",
                     font=ctk.CTkFont("Poppins", 14, "bold"), text_color="#FFF", padx=8).pack(side="left")
